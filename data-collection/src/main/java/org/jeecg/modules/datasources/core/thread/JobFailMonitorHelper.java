...
public class JobFailMonitorHelper {
					...
					try {
						...
						if (failLogIds!=null && !failLogIds.isEmpty()) {
							for (long failLogId: failLogIds) {
								...
								WaterfallJobLog log = JobAdminConfig.getAdminConfig().getJobLogMapper().load(failLogId);
								WaterfallJobInfo info = JobAdminConfig.getAdminConfig().getJobInfoMapper().loadById(log.getJobId());
	 ...
	 * @param jobLog
	 */
	private boolean failAlarm(WaterfallJobInfo info, WaterfallJobLog jobLog){
		boolean alarmResult = true;
		...
		if (info!=null && info.getAlarmEmail()!=null && info.getAlarmEmail().trim().length()>0) {
			...
			// email info
			WaterfallJobGroup group = JobAdminConfig.getAdminConfig().getJobGroupMapper().load(Integer.valueOf(info.getJobGroup()));
